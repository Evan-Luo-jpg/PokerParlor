...
// Assuming primes and bit constants from Suffecool's arrays.h

int encodeCard(const Card& card) {
    int rank = static_cast<int>(card.getRank()) - 2; // Map TWO (2) to index 0
    int suitMask;

    switch (card.getSuit()) {
        case CLUBS: suitMask = 0x8000; break;
        case DIAMONDS: suitMask = 0x4000; break;
        case HEARTS: suitMask = 0x2000; break;
        case SPADES: suitMask = 0x1000; break;
        default: suitMask = 0; break;
    }
